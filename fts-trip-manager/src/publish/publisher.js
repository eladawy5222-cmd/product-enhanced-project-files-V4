...
function mapAirtableToWordPress_(data, tripFields) {
  ...
  if (data.includes.length > 0) {
    ...
    var excludesLines = (data.excludes || []).map(function (r) { return r && r.fields ? String(r.fields.ExcludeItem || '') : '' }).map(function (t) { return pub_normLine_(t) }).filter(Boolean)
  ...
  if (data.excludes.length > 0 && !wte.cost.cost_excludes) {
