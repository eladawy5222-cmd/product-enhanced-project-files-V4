...
async function fetchCompleteTripData_Updater_(tripId, tripFields, opts) {
  ...
  if (!data.addons || !data.addons.length) {
    try {
      data.addonsRaw = await findRecordsByLinkedId_Updater_('AddOns', 'Trip', tripId)
    } catch (e) {
      data.addonsRaw = []
    }
  }
...
async function publishPackagesSafe_Updater_(tripId, wpTripId, opts) {
   ...
   try {
    ...
    if (pkgSanitizeEnabled) {
      try {
        var incRecsPkg = await findRecordsByLinkedId_Updater_('TripIncludes Improvement With AI', 'Trip', tripId)
        var excRecsPkg = await findRecordsByLinkedId_Updater_('TripExcludes Improvement With AI', 'Trip', tripId)
