...
function initIncExc(options) {
  if (!options) throw new Error('createIncExcEnhancer: missing options')
  ...
  if (!airtable) throw new Error('createIncExcEnhancer: missing options.airtable')
  if (!http) throw new Error('createIncExcEnhancer: missing options.http')
  if (!config) throw new Error('createIncExcEnhancer: missing options.config')
  if (!logger) throw new Error('createIncExcEnhancer: missing options.logger')
  if (!store) throw new Error('createIncExcEnhancer: missing options.store')
...
function escapeSingleQuotes_(s) {
  return String(s == null ? '' : s).replace(/'/g, "\\'")
}

async function fetchRecordsByTrip_(tableName, tripRecordId, tripPublicId, limit, tripName) {
  const t = String(tableName || '')
  const lf = (config && config.LINK_FIELDS && config.LINK_FIELDS[t])
    ? String(config.LINK_FIELDS[t])
    : (config && config.DEFAULT_TRIP_LINK_FIELD ? String(config.DEFAULT_TRIP_LINK_FIELD) : 'Trip')

  const parts = []
  if (tripRecordId) parts.push(`FIND('${escapeSingleQuotes_(tripRecordId)}', ARRAYJOIN({${lf}}))`)
  if (tripPublicId) parts.push(`FIND('${escapeSingleQuotes_(tripPublicId)}', ARRAYJOIN({${lf}}))`)
  if (tripName) parts.push(`FIND('${escapeSingleQuotes_(tripName)}', ARRAYJOIN({${lf}}))`)
...
async function buildUnifiedTripContext_(tripId, tripFields) {
  const tripPublicId = tripFields && tripFields.TripID ? String(tripFields.TripID) : ''
  const tripName = tripFields && tripFields.Title ? String(tripFields.Title) : ''

  const highlightsImp = await fetchRecordsByTrip_('Highlights Improvement With AI', tripId, tripPublicId, 100, tripName)
  const itineraryImp = await fetchRecordsByTrip_('Itinerary Improvement With AI', tripId, tripPublicId, 100, tripName)
  const addonsImp = await fetchRecordsByTrip_('AddOns Improvement With AI', tripId, tripPublicId, 100, tripName)
  ...
  const highlightsText = (highlightsImp || [])
    .map((r) => String(((r && r.fields) ? r.fields.AI_Highlight : '') || '').trim())
    .filter(Boolean)
    .join('\n')

  const itineraryText = (itineraryImp || [])
    .map((r) => {
      const f = r && r.fields ? r.fields : {}
      const t = String(f.AI_Step_Title || '').trim()
      const d = String(f.AI_Step_Description || '').trim()
      const meals = String(f.AI_Meals_Included || '').trim()
      const label = String(f.AI_Step_Label || '').trim()
      if (!t && !d && !meals && !label) return ''
      let line = t
      if (d) line += (line ? ' | ' : '') + d
  ...
  const addonsText = (addonsImp || [])
    .map((r) => {
      const f = r && r.fields ? r.fields : {}
      const title = String(f.AI_AddOn_Title || '').trim()
      const price = f.AI_AddOn_Price != null ? String(f.AI_AddOn_Price).trim() : ''
      const desc = String(f.AI_AddOn_Description || '').trim()
  ...
  const packagesText = (packages || [])
    .map((r) => {
      const f = r && r.fields ? r.fields : {}
      const t = String(f.PackageName || f.PackageTitle || f.Title || f.Name || '').trim()
      const d = String(f.ShortDescription || f.Description || '').trim()
      if (!t && !d) return ''
      return t ? (d ? `${t}: ${d}` : t) : d
    })
    .filter(Boolean)
  ...
  const pickupText = (pickup || [])
    .map((r) => {
      const f = r && r.fields ? r.fields : {}
      const name = String(f.LocationName || f.Location || f.Title || f.Name || '').trim()
      const notes = String(f.LocationNotes || f.Notes || '').trim()
  ...
  return { highlightsText, itineraryText, addonsText, detailsText, packagesText, faqsText, pickupText }
}
...
async function fetchMainAiImprovementForTrip_(tripId, tripNumber) {
  ...
  var res = await airtableGet_(tableName, { filterByFormula: formula, maxRecords: 1 })
...
async function callAi_(prompt) {
  if (!aiProvider) throw new Error('AI Inc/Exc: missing aiProvider')
  if (typeof aiProvider.callDeepseek === 'function') return aiProvider.callDeepseek(String(prompt || ''))
  if (typeof aiProvider.callDeepSeekJson === 'function') return aiProvider.callDeepSeekJson(String(prompt || ''))
  throw new Error('AI Inc/Exc: aiProvider missing callDeepseek/callDeepSeekJson')
}
 ...
 * ENTRY POINT
 */
async function runAiIncludesExcludesExtractionBatch() {
  log('AI Inc/Exc: starting batch...');

  try {
    const trips = await fetchTripsNeedingIncExc_(AI_INCEXC_BATCH_LIMIT)

    if (!trips || !trips.length) {
      log('AI Inc/Exc: no Trips with ' + TRIP_INCEXC_STATUS_FIELD + " = 'Pending'.");
      return;
    }

    for (let ti = 0; ti < trips.length; ti++) {
      const tripRec = trips[ti]
      const tripId = tripRec.id
      const fields = tripRec.fields || {}
      const tripNumber = fields.TripID || ''
      const tripName = fields.Title || ''

      try {
        log('AI Inc/Exc: processing Trip ' + tripId + ' (TripID: ' + tripNumber + ')');

        // 1) علامة "Processing"
        await updateTripIncExcStatus_(tripId, 'Processing')
        
        if (!tripNumber) {
          log('⚠️ Warning: TripID is missing for Trip ' + tripId);
        }

        // 2) بناء Context
        const ctx = await buildTripIncExcContext_(fields, tripId, tripNumber)

        const U = await buildUnifiedTripContext_(tripId, fields)
        const combinedText = (ctx.tripDescription || '') +
        ...
        const rawIncludes = await fetchRawIncludesForTrip_(tripId, tripNumber, tripName)
        const rawExcludes = await fetchRawExcludesForTrip_(tripId, tripNumber, tripName)

        // 🔍 LOGGING RAW DATA (User Request)
        log('🔍 DATA CHECK [Trip ' + tripId + ']: Found ' + rawIncludes.length + ' items in TripIncludes (Raw).');
        if (rawIncludes.length > 0) {
          log('   Sample Raw Includes: ' + rawIncludes.slice(0, 3).join(' | '));
        } else {
          log('   ⚠️ Warning: TripIncludes (Raw) is empty! This is the primary source.');
        }

        log('🔍 DATA CHECK [Trip ' + tripId + ']: Found ' + rawExcludes.length + ' items in TripExcludes (Raw).');
        if (rawExcludes.length > 0) {
          log('   Sample Raw Excludes: ' + rawExcludes.slice(0, 3).join(' | '));
        } else {
          log('   ⚠️ Warning: TripExcludes (Raw) is empty!');
        }
        ...
        if (!combinedText.trim() && (!rawIncludes.length && !rawExcludes.length) && !(linkedTextBlocks && linkedTextBlocks.length)) {
          log('AI Inc/Exc: Trip ' + tripId + ' has no cost lists or text; marking as Empty.');
        ...
        const improvedAddOnsData = await fetchImprovedAddOnsDataForTrip_(tripId, tripNumber, tripName)

        // 4) بناء الـ Prompt مع مراعاة الاختيارية
        const prompt = buildIncExcExtractionPrompt_(ctx, rawIncludes, rawExcludes, linkedTextBlocks, improvedHighlights, improvedItinerary, improvedAddOns)
        ...
        let includes = aiResult.includes || []
        let excludes = aiResult.excludes || []
        ...
        if (excludes.length < MIN_EXCLUDES_COUNT) {
          ...
          log('⚠️ Adding ' + needed + ' default excludes to meet minimum requirement');
        ...
        if (excludes.length < MIN_EXCLUDES_COUNT) {
          ...
          log('⚠️ After filtering, adding ' + stillNeeded + ' more default excludes');
        ...
        if (includes.length < MIN_INCLUDES_RECOMMENDED) {
          ...
          log('⚠️ Only ' + includes.length + ' includes. Adding ' + includesNeeded + ' default includes');
        ...
        let createdIncludesCount = 0
        let createdExcludesCount = 0

        // إنشاء Includes الجديدة
        for (let ii = 0; ii < includes.length; ii++) {
          const text = includes[ii]
          ...
          await airtableCreate_(TRIP_INCLUDES_IMPROVEMENT_TABLE, fieldsCreate)
          createdIncludesCount++
        }

        // إنشاء Excludes الجديدة
        for (let ei = 0; ei < excludes.length; ei++) {
          const text = excludes[ei]
          const trimmed = (text || '').toString().trim()
          if (!trimmed) continue
          const nowIso = new Date().toISOString()
          const fieldsCreate = {}
          ...
          fieldsCreate.AI_Status = 'Done'
          fieldsCreate.AI_LastUpdated = nowIso
          await airtableCreate_(TRIP_EXCLUDES_IMPROVEMENT_TABLE, fieldsCreate)
          createdExcludesCount++
        }
 log('AI Inc/Exc: Trip ' + tripId +
           ' → includes: ' + createdIncludesCount +
           ', excludes: ' + createdExcludesCount);

        // 6) تحديث الحالة
        await updateTripIncExcStatus_(tripId, 'Done')



      } catch (e) {
        log('AI Inc/Exc: error for Trip ' + tripId + ' — ' + e.message);
        await updateTripIncExcStatus_(tripId, 'Error')
      }
    }

  } catch (e) {
    log('AI Inc/Exc: fatal error — ' + e.message);
  }

  log('AI Inc/Exc: batch finished.');
}
...
async function buildTripIncExcContext_(fields, tripId, tripNumber) {
  var ctx = {
    title:                 fields.Title || '',
...
async function createTripIncludeImprovementRecord_(tripId, text) {
  const nowIso = new Date().toISOString()
  const fields = {}
  ...
  const created = await airtableCreate_(TRIP_INCLUDES_IMPROVEMENT_TABLE, fields)
  return created && created.id ? created.id : null
}

async function createTripExcludeImprovementRecord_(tripId, text) {
  const nowIso = new Date().toISOString()
  const fields = {}
  ...
  const created = await airtableCreate_(TRIP_EXCLUDES_IMPROVEMENT_TABLE, fields)
  return created && created.id ? created.id : null
}
...
async function updateTripIncExcStatus_(tripId, status) {
  if (!tripId) return;
  const fields = {}
...
async function createAiIncludesExcludesExtractorTrigger() {
  log('AI Inc/Exc: createAiIncludesExcludesExtractorTrigger is not supported in Node runtime')
}

async function fetchRawTripIncludes_(tripId, tripNumber, tripFields) {
  if (!tripId) return [];
  const out = []
  ...
  for (let i = 0; i < (recs || []).length; i++) {
    ...
    const t = (f[INCLUDE_TEXT_FIELD] || '').toString().trim()
    if (t) out.push(t)
  }
  return out;
}

async function fetchRawTripExcludes_(tripId, tripNumber, tripFields) {
  if (!tripId) return [];
  const out = []
  ...
  for (let i = 0; i < (recs || []).length; i++) {
    const f = recs[i].fields || {}
    const t = (f[EXCLUDE_TEXT_FIELD] || '').toString().trim()
    if (t) out.push(t)
  }
  return out;
}
...
function fetchImprovedItineraryForTrip_(tripId) {
  ...
  try {
    ...
    log("fetchImprovedItineraryForTrip_ error: " + e.message);
    return "";
  }
}
...
function fetchImprovedAddOnsDataForTrip_(tripId, tripNumber, tripName) {
  ...
  try {
    ...
    log("fetchImprovedAddOnsDataForTrip_ error: " + e.message);
    return [];
  }
}
...
function reformatAddOnsInIncludes_(includes, addOnsData, tripId) {
  ...
  inputList.forEach(function(item) {
    ...
    if (bestScore >= 3 && bestMatch) {
      ...
      log('✅ REFORMATTED ADDON [Trip ' + tripId + '] Score: ' + bestScore + ' | "' + text + '" → "' + formattedAddOn + '"');
    }
  ...
  addOnsData.forEach(function(addOn) {
    if (!matchedAddOnTitles[addOn.title]) {
      ...
      if (looseMatchIndex !== -1) {
         ...
         log('🔄 FORCE MATCH REPLACE [Trip ' + tripId + ']: Replacing loose match "' + reformatted[looseMatchIndex] + '" with Official "' + formattedAddOn + '"');
         reformatted[looseMatchIndex] = formattedAddOn;
         matchedAddOnTitles[addOn.title] = true;
      } else {
         // ➕ FORCE ADD: لم يتم العثور على أي أثر، أضفه
         reformatted.push(formattedAddOn);
         log('➕ FORCE ADDED ADDON [Trip ' + tripId + ']: "' + formattedAddOn + '" (Was missing from AI output)');
      }
    }
  });
  
  if (reformattedCount > 0) {
    log('✅ REFORMAT SUMMARY [Trip ' + tripId + ']: Reformatted ' + reformattedCount + ' AddOns to correct format');
  }
...
function sanitizeTransportationWords_(items, tripId, rawText) {
    ...
    for (var i = 0; i < FORBIDDEN_WORDS.length; i++) {
      ...
      if (lower.indexOf(forbiddenWord) !== -1) {
        ...
        if (rawTextLower.indexOf(forbiddenWord) !== -1) {
           log('ℹ️ ALLOWING forbidden word "' + forbiddenWord + '" in item "' + text + '" because it exists in RAW source.');
           continue; // مسموح بها لأنها في المصدر
        }

        hasForbiddenWord = true;
        log('⚠️ SANITIZATION WARNING [Trip ' + tripId + ']: Forbidden word "' + 
  ...
  if (replacedCount > 0) {
    log('✅ SANITIZATION SUMMARY [Trip ' + tripId + ']: Replaced ' + 
...
function deleteOldIncludesExcludesForTrip_(tripId, tripNumber) {
  ...
  try {
    ...
    if (deletedIncludes === 0 && deletedExcludes === 0) {
      log('ℹ️ AI Inc/Exc: no old records to delete for Trip ' + tripId);
    } else {
      log('✅ AI Inc/Exc: SUCCESSFULLY deleted ' + deletedIncludes + ' includes and ' + deletedExcludes + ' excludes for Trip ' + tripId);
    }
  } catch (e) {
    log('❌ AI Inc/Exc: ERROR deleting old records for Trip ' + tripId + ': ' + e.message);
  }
}
...
function enforceCountLimitsWithGrouping_(items, minCount, maxCount, type, tripId) {
  ...
  if (count < minCount) {
    log('⚠️ WARNING [Trip ' + tripId + ']: Only ' + count + ' ' + type + 
...
function smartGroupItems_(items, maxCount, type) {
  ...
  if (result.length > maxCount) {
    log('⚠️ Still exceeding max after grouping. Taking top ' + maxCount + ' items.');
...
function deduplicateSimilarItems_(items, type, tripId) {
    ...
    for (var i = 0; i < result.length; i++) {
      ...
      if (similarity > 0.7) {
        ...
        if (itemIsOptional && !existingIsOptional) {
          ...
          log('🔄 DEDUPLICATION [Trip ' + tripId + ']: Replacing "' + result[i] + '" with OPTIONAL "' + item + '"');
          result[i] = item;
        } else if (!itemIsOptional && existingIsOptional) {
          // القديم هو الاختياري، احتفظ به
          log('🔄 DEDUPLICATION [Trip ' + tripId + ']: Keeping OPTIONAL "' + result[i] + '" over "' + item + '"');
        } else {
          // كلاهما اختياري أو كلاهما عادي -> طبق قاعدة الأقصر
          if (item.length < result[i].length) {
            log('🔄 DEDUPLICATION [Trip ' + tripId + ']: Replacing "' + result[i] + '" with shorter "' + item + '"');
            result[i] = item;
          } else {
            log('🔄 DEDUPLICATION [Trip ' + tripId + ']: Skipping duplicate "' + item + '"');
          }
        }
  ...
  if (duplicatesRemoved > 0) {
    log('✅ DEDUPLICATION [Trip ' + tripId + ']: Removed ' + duplicatesRemoved + 
  ...
  items.forEach(function(item) {
    ...
    for (var i = 0; i < INAPPROPRIATE_KEYWORDS.length; i++) {
      if (lower.indexOf(INAPPROPRIATE_KEYWORDS[i]) !== -1) {
        ...
        log('🚫 FILTER [Trip ' + tripId + ']: Removed inappropriate exclude: "' + item + '"');
        removedCount++;
        break;
      }
    }
...
function filterConflictingExcludes_(excludes, includes, rawExcludes, tripId) {
  ...
  excludes.forEach(function(item) {
    ...
    if (lower.indexOf('entrance') !== -1 || lower.indexOf('admission') !== -1 || lower.indexOf('ticket') !== -1) {
      ...
      if (!explicitlyExcludedRaw) {
        log('🚫 CONFLICT FILTER [Trip ' + tripId + ']: Removed "' + item + '" from Excludes (Not in Raw Excludes)');
        removedCount++;
      ...
      if (implicitlyIncluded) {
        log('🚫 CONFLICT FILTER [Trip ' + tripId + ']: Removed "' + item + '" from Excludes (Found in Includes)');
        removedCount++;
        return; // Skip adding this item
      }
    }
  ...
  if (removedCount > 0) {
    log('✅ CONFLICT RESOLUTION [Trip ' + tripId + ']: Removed ' + removedCount + ' conflicting items from Excludes.');
  }
...
function fetchRawIncludesForTrip_(tripId, tripNumber, tripName) {
  var items = [];
  try {
    log('AI Inc/Exc: fetching raw Includes for Trip ' + tripId + ' (TripID: ' + tripNumber + ', Name: ' + tripName + ')');
    var conditions = [];
    if (tripName) {
      var safeName = tripName.replace(/'/g, "\\'");
      conditions.push("FIND('" + safeName + "', ARRAYJOIN({Trip}))");
    ...
    log('AI Inc/Exc: Includes filter formula: ' + formula);
    ...
    log('AI Inc/Exc: found ' + records.length + ' raw Includes records');
    ...
    log('AI Inc/Exc: error fetching raw Includes: ' + e.message);
  }
  return items;
}
...
function fetchRawExcludesForTrip_(tripId, tripNumber, tripName) {
  var items = [];
  try {
    log('AI Inc/Exc: fetching raw Excludes for Trip ' + tripId + ' (TripID: ' + tripNumber + ', Name: ' + tripName + ')');
    var conditions = [];
    if (tripName) {
      var safeName = tripName.replace(/'/g, "\\'");
      conditions.push("FIND('" + safeName + "', ARRAYJOIN({Trip}))");
    ...
    log('AI Inc/Exc: Excludes filter formula: ' + formula);
    var res = airtableGet_(TRIP_EXCLUDES_BASE_TABLE, { filterByFormula: formula, pageSize: 100 });
    var records = res && res.records ? res.records : [];
    
    log('AI Inc/Exc: found ' + records.length + ' raw Excludes records');
    records.forEach(function(rec) {
      var f = rec.fields || {};
    ...
    log('AI Inc/Exc: error fetching raw Excludes: ' + e.message);
  }
  return items;
}

function createIncExcEnhancer(options) {
  let inited = false
  ...
  return {
    runAiIncludesExcludesExtractionBatch: async (...args) => {
      await ensureInit()
      return runAiIncludesExcludesExtractionBatch(...args)
    }
  }
}

module.exports = { createIncExcEnhancer }
