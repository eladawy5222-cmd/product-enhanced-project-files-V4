...
function initTripFactsEnhancer(options) {
  ...
  if (!airtable) throw new Error('createTripFactsEnhancer: missing options.airtable')
  if (!http) throw new Error('createTripFactsEnhancer: missing options.http')
  if (!config) throw new Error('createTripFactsEnhancer: missing options.config')
  if (!logger) throw new Error('createTripFactsEnhancer: missing options.logger')
  if (!store) throw new Error('createTripFactsEnhancer: missing options.store')
  if (!aiProvider) throw new Error('createTripFactsEnhancer: missing options.aiProvider')
...
async function buildUnifiedTripContext_(tripId, tripFields) {
  ...
  const itineraryText = itinerary
    .map((r) => {
      const f = r.fields || {}
      const t = String(f.AI_Step_Title || f.StepTitle || '').trim()
      const d = String(f.AI_Step_Description || f.StepDescription || '').trim()
      if (!t && !d) return ''
      return t ? (d ? `${t}: ${d}` : t) : d
    })
    .filter(Boolean)
  ...
  return { highlightsText, itineraryText }
}
 ...
 * MAIN BATCH FUNCTION
 ************************************************************/
async function runAiTripFactsEnhancementBatch() {
  loadConfigSecrets_();
  log('AI TripFacts: starting batch...');
  ...
  if (!trips || !trips.length) {
    log('AI TripFacts: no Trips with ' + TRIP_FACTS_STATUS_FIELD + " = 'Pending'.");
    return;
  }

  // 🧩 Load dictionary of all possible facts (FactId + FactKey + FactLabel)
  var rawFacts = await fetchRawTripFacts_()
  ...
  for (let i = 0; i < trips.length; i++) {
    ...
    try {
      log('AI TripFacts: processing Trip ' + tripId);

      // 1) Update status to Processing
      await updateTripFactsStatus_(tripId, 'Processing')

      // 2) Delete old improved facts
      await deleteOldTripFactsForTrip_(tripId, fields.TripID || '')

      // 3) Build context
      var ctx = await buildTripFactsContext_(fields, tripId)
      ...
      for (let fIdx = 0; fIdx < facts.length; fIdx++) {
        ...
        await airtableCreate_(TRIP_FACTS_IMPROVEMENT_TABLE, fieldsCreate)
        createdCount++;
      }

      log('AI TripFacts: Trip ' + tripId + ' → created ' + createdCount + ' facts');

      // 7) Update status
      await updateTripFactsStatus_(tripId, 'Done')

    } catch (e) {
      log('AI TripFacts: error for Trip ' + tripId + ' — ' + e.message);
      await updateTripFactsStatus_(tripId, 'Error')
    }
  }

  log('AI TripFacts: batch finished.');
}

/************************************************************
 * CONTEXT BUILDING
 ************************************************************/
async function buildTripFactsContext_(fields, tripId, improvedFields) {
  var ctx = {};

  // Fetch improved AI content (passing TripID and Title for robust search)
  var improved = await fetchImprovedDataForTrip_(tripId, fields.TripID, fields.Title)
  ...
  if (improvedFields && improvedFields.Duration_Hours) {
    ...
    log('AI TripFacts: Using Improved Duration (Arg): ' + ctx.durationHours + 'h ' + ctx.durationMinutes + 'm ' + ctx.durationUnit);
    ...
    log('AI TripFacts: Using Improved Duration (Fetched): ' + ctx.durationHours + 'h ' + ctx.durationMinutes + 'm ' + ctx.durationUnit);
  } else {
  ...
  if (ctx.durationHours && ctx.durationUnit) {
    ...
    if (unit === 'days' && hours > 30) {
      log('⚠️ DURATION WARNING [Trip ' + tripId + ']: ' + hours + ' days seems unrealistic. Using generic value.');
      ctx.durationHours = '';
      ctx.durationUnit = 'Multi-day tour';
    } else if (unit === 'hours' && hours > 720) {
      log('⚠️ DURATION WARNING [Trip ' + tripId + ']: ' + hours + ' hours seems unrealistic. Converting to days.');
  ...
  var U = await buildUnifiedTripContext_(tripId, fields)
...
function normalizeFactKeysArray_(facts, tripId) {
  ...
  facts.forEach(function(fact, idx) {
    ...
    if (!normalized && rawKey) {
      log('⚠️ NORMALIZE [Trip ' + tripId + ']: Unsupported FactKey "' + rawKey + '" - this fact may be skipped.');
    }
...
function sanitizeFactValues_(facts, tripId) {
  ...
  facts.forEach(function(fact, idx) {
    ...
    if (value.toLowerCase().indexOf('not specified') !== -1 ||
      ...
      if (defaultValue) {
        log('🔧 SANITIZE [Trip ' + tripId + ']: Replacing \"'
                   + value + '\" with \"' + defaultValue + '\" for ' + fact.label);
        facts[idx].value = defaultValue;
        sanitizedCount++;
      } else {
        log('⚠️ SANITIZE [Trip ' + tripId + ']: No default for ' + fact.label + ', using generic value');
  ...
  if (sanitizedCount > 0) {
    log('✅ SANITIZE [Trip ' + tripId + ']: Replaced ' + sanitizedCount + ' \"Not specified\" values');
  }
...
async function deleteOldTripFactsForTrip_(tripId, tripCode) {
  if (!tripId) return;
 ...
 */
async function fetchRawTripFacts_() {
  var rawFacts = [];
  var offset = null;
  ...
  try {
    do {
      ...
      var res = await airtableGet_(TRIP_FACTS_BASE_TABLE, params)
      var recs = res && res.records ? res.records : [];
      offset = res ? res.offset : null;

      log('AI TripFacts: loaded ' + recs.length +
    ...
    log('AI TripFacts: error fetching raw facts dictionary: ' + e.message);
  }
...
function matchFactId_(rawFacts, label, key) {
  ...
  for (var i = 0; i < rawFacts.length; i++) {
    ...
    if (raw.label && raw.label.toLowerCase().trim() === labelLower) {
      log('AI TripFacts: Matched FactId ' + raw.factId +
  ...
  for (var j = 0; j < rawFacts.length; j++) {
    ...
    if (rawKeyCanonical && rawKeyCanonical === keyLower) {
      log('AI TripFacts: Matched FactId ' + raw2.factId +
  ...
  if (KNOWN_FACT_IDS[keyLower]) {
    log('AI TripFacts: Matched FactId ' + KNOWN_FACT_IDS[keyLower] +
...
async function fetchImprovedDataForTrip_(tripId, tripNumber, tripName) {
  var result = {
    description: '',
    overview: '',
    highlights: '',
  ...
  try {
    var recs = await fetchRecordsByTrip_(IMPROVEMENT_TABLE, tripId, tripNumber, 1)
    ...
    // Fetch highlights
    var highlightsRes = await fetchRecordsByTrip_(HIGHLIGHTS_IMPROVEMENT_TABLE, tripId, tripNumber, 100)
    ...
    log('AI TripFacts: error fetching improved data for Trip ' + tripId + ': ' + e.message);
  }
 ...
 * TRIGGER SETUP
 ************************************************************/
async function createAiTripFactsEnhancerTrigger() {
  log('AI TripFacts: createAiTripFactsEnhancerTrigger is not supported in Node runtime')
}

function createTripFactsEnhancer(options) {
  let inited = false
  async function ensureInit() {
    if (inited) return
    initTripFactsEnhancer(options)
    inited = true
  }

  return {
    runAiTripFactsEnhancementBatch: async (...args) => {
      await ensureInit()
      return runAiTripFactsEnhancementBatch(...args)
    }
  }
}

module.exports = { createTripFactsEnhancer }
