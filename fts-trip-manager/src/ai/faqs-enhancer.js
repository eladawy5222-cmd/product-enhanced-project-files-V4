...
function initFaqsEnhancer(options) {
  if (!options) throw new Error('createFaqsEnhancer: missing options')
  ...
  if (!airtable) throw new Error('createFaqsEnhancer: missing options.airtable')
  if (!http) throw new Error('createFaqsEnhancer: missing options.http')
  if (!config) throw new Error('createFaqsEnhancer: missing options.config')
  if (!logger) throw new Error('createFaqsEnhancer: missing options.logger')
  if (!store) throw new Error('createFaqsEnhancer: missing options.store')
  if (!aiProvider) throw new Error('createFaqsEnhancer: missing options.aiProvider')
...
async function buildUnifiedTripContext_(tripId, tripFields) {
  ...
  const imp = await fetchRecordsByTrip_(IMPROVEMENT_TABLE, tripId, tripPublicId, 1, tripName)
  const rawHighlights = await fetchRecordsByTrip_('TripHighlights', tripId, tripPublicId, 50, tripName)
  const highlights = await fetchRecordsByTrip_(HIGHLIGHTS_IMPROVEMENT_TABLE, tripId, tripPublicId, 50, tripName)
  const itinerary = await fetchRecordsByTrip_(ITINERARY_IMPROVEMENT_TABLE, tripId, tripPublicId, 100, tripName)
  const includes = await fetchRecordsByTrip_(TRIP_INCLUDES_IMPROVEMENT_TABLE, tripId, tripPublicId, 100, tripName)
  const excludes = await fetchRecordsByTrip_(TRIP_EXCLUDES_IMPROVEMENT_TABLE, tripId, tripPublicId, 100, tripName)
  const tripFacts = await fetchRecordsByTrip_(TRIPFACTS_IMPROVEMENT_TABLE, tripId, tripPublicId, 50, tripName)

  const impFields = imp && imp.length ? (imp[0].fields || {}) : {}

  const rawHighlightsArr = rawHighlights
    .map((r) => {
      const f = r.fields || {}
      return String(f.Highlight || f.Title || '').trim()
    })
    .filter(Boolean)

  const improvedHighlightsArr = highlights
  ...
  const excludesArr = excludes
    .map((r) => {
      const f = r.fields || {}
      return String(f.AI_ExcludeItem || f.AI_ExcludeText || '').trim()
    })
    .filter(Boolean)

  const tripFactsArr = tripFacts
    .map((r) => {
      const f = r.fields || {}
      const label = String(f.AI_Fact_Label || '').trim()
  ...
  return {
    aiTripDescription: String(impFields.AI_Trip_Description || '').trim(),
    aiOverviewTitle: String(impFields.AI_Overview_Section_Title || '').trim(),
    focusKeyword: String(impFields.AI_SEO_FocusKeywords || '').trim(),
    rawHighlightsArr,
 ...
 * MAIN BATCH FUNCTION
 ************************************************************/
async function runAiFaqsEnhancementBatch() {
  loadConfigSecrets_();
  log('AI FAQs Generator: starting batch...');
  ...
  try {
    ...
    if (!trips || !trips.length) {
      log('AI FAQs: no trips with ' + FAQS_STATUS_FIELD + " = 'Pending'.");
      return;
    }
    ...
    for (let i = 0; i < trips.length; i++) {
      ...
      try {
        log('AI FAQs: processing Trip ' + tripId);
        
        // 1) Update status to Processing
        await updateTripFaqsStatus_(tripId, 'Processing')
        
        // 2) Delete old FAQs for this trip
        await deleteOldFaqsForTrip_(tripId, tripFields.TripID || '')
        
        // 3) Build comprehensive context
        var ctx = await buildFaqsContext_(tripFields, tripId)
        ...
        if (faqs.length < MIN_FAQS_COUNT) {
          log('AI FAQs: WARNING - only ' + faqs.length + ' FAQs generated (minimum target is ' + MIN_FAQS_COUNT + ').');
          faqs = ensureMinimumFaqs_(faqs, ctx);
        }
        
        if (faqs.length > MAX_FAQS_COUNT) {
          log('AI FAQs: trimming from ' + faqs.length + ' to ' + MAX_FAQS_COUNT + ' FAQs');
          faqs = faqs.slice(0, MAX_FAQS_COUNT);
        }
        
        // 7) Create FAQ records
        var createdCount = 0;
        for (let fIdx = 0; fIdx < faqs.length; fIdx++) {
          const faq = faqs[fIdx]
          try {
            ...
            if (!question || !answer) {
              log('AI FAQs: skipping FAQ with empty question or answer');
              return;
            }
            ...
            await airtableCreate_(FAQS_IMPROVEMENT_TABLE, fieldsCreate)
            createdCount++;
            
          } catch (e) {
            log('AI FAQs: error creating FAQ — ' + e.message);
          }
        }
        ...
        log('AI FAQs: error for Trip ' + tripId + ' — ' + e.message);
        await updateTripFaqsStatus_(tripId, 'Error')
      }
    }
    
  } catch (e) {
    log('AI FAQs: fatal error — ' + e.message);
  }
  
  log('AI FAQs: batch finished.');
}

/************************************************************
 * CONTEXT BUILDING
 ************************************************************/
async function buildFaqsContext_(tripFields, tripId) {
  var ctx = {};
  ...
  try {
    ...
    log('AI FAQs: error fetching improved data: ' + e.message);
  }
  ...
  try {
    ...
    log('AI FAQs: error fetching trip facts: ' + e.message);
  }
  ...
  try {
    ...
    log('AI FAQs: error fetching images: ' + e.message);
  }
...
function upsertCancellationFaq_(faqs, ctx) {
  ...
  log('AI FAQs: Checking duration for cancellation policy. Raw Duration: "' + durationRaw + '"');
  ...
  function getDaysFromText(text) {
    ...
    if (s.indexOf('hour') !== -1 || s.indexOf('min') !== -1) {
      log('AI FAQs: Duration "' + text + '" implies short trip (hours/mins).');
  ...
  if (days === 0 && ctx && Array.isArray(ctx.facts)) {
     ...
     log('AI FAQs: Checking Facts for duration: ' + factsText);
  ...
  if (days === -1) {
     days = 0;
      ...
      if (days === 0 && ctx && ctx.tripTitle) {
         log('AI FAQs: Duration field inconclusive, checking Title: "' + ctx.tripTitle + '"');
      ...
      if (days === 0 && ctx && Array.isArray(ctx.itinerary) && ctx.itinerary.length > 0) {
         ...
         log('AI FAQs: Duration/Title inconclusive, checking Itinerary Items: ' + ctx.itinerary.length);
  ...
  if (days > 3) {
    ...
    log('AI FAQs: Detected short trip (' + days + ' days). Using FLEXIBLE policy.');
  }
 ...
 */
async function deleteOldFaqsForTrip_(tripId, tripNumber) {
  if (!tripId) return;
  ...
  try {
    ...
    if (toDelete.length > 0) {
      log('AI FAQs: deleting ' + toDelete.length + ' old FAQs for Trip ' + tripId);
      for (var j = 0; j < toDelete.length; j++) {
        var id = toDelete[j]
        try {
          await airtableDelete_(FAQS_IMPROVEMENT_TABLE, id)
        } catch (e) {
          log('AI FAQs: failed to delete FAQ ' + id + ' — ' + e.message)
        }
      }
    }
  } catch (e) {
    log('AI FAQs: error deleting old FAQs for Trip ' + tripId + ': ' + e.message);
  }
}
...
function ensureMinimumFaqs_(faqs, ctx) {
  ...
  if (added) log('AI FAQs: added ' + added + ' context-supported FAQs (no generic padding)');
  return faqs;
}
 ...
 */
async function updateTripFaqsStatus_(tripId, status) {
  if (!tripId) return;
  var fields = {};
 ...
 * TRIGGER SETUP
 ************************************************************/
async function createAiFaqsEnhancerTrigger() {
  log('AI FAQs: createAiFaqsEnhancerTrigger is not supported in Node runtime')
}

function createFaqsEnhancer(options) {
  let inited = false
  async function ensureInit() {
    if (inited) return
    initFaqsEnhancer(options)
    inited = true
  }

  return {
    runAiFaqsEnhancementBatch: async (...args) => {
      await ensureInit()
      return runAiFaqsEnhancementBatch(...args)
    }
  }
}

module.exports = { createFaqsEnhancer }
