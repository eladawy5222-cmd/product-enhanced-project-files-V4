...
function initSeoEnhancer(options) {
  if (!options) throw new Error('createSeoEnhancer: missing options')
  ...
  if (!airtable) throw new Error('createSeoEnhancer: missing options.airtable')
  if (!http) throw new Error('createSeoEnhancer: missing options.http')
  if (!config) throw new Error('createSeoEnhancer: missing options.config')
  if (!logger) throw new Error('createSeoEnhancer: missing options.logger')
  if (!store) throw new Error('createSeoEnhancer: missing options.store')
  if (!aiProvider) throw new Error('createSeoEnhancer: missing options.aiProvider')
...
function filterEnglishKeywordsWithLocaleDiagnostics_(rawList, label) {
  ...
  if (rejectedLocale) log('AI SEO Enhancer: rejected locale phrases (' + label + '): ' + rejectedLocale + (examples.length ? (' examples=' + JSON.stringify(examples)) : ''));
...
async function cleanEnglishKeywordsWithAiSeoEn_(combinedFields, linkedTextBlocks) {
  var rawPool = buildRawKeywordCandidatePoolSeoEn_(combinedFields)
  log('AI SEO Enhancer: keyword candidates pool=' + rawPool.length)
  if (!rawPool.length) return { primary: '', list: [] }

  var prompt = buildKeywordCleaningPromptSeoEn_(combinedFields, linkedTextBlocks, rawPool)
  var ai = null
  try { ai = await callAi_(prompt) } catch { ai = null }
  if (!ai || typeof ai !== 'object') {
    log('AI SEO Enhancer: keyword cleaning AI output invalid; using fallback')
  ...
  if (preserved > 0) log('AI SEO Enhancer: preserved valid English phrases from source pool: ' + preserved)
  aiList = mergedList
  if (!aiPrimary && aiList.length) aiPrimary = normalizeEnglishSeoKeywordPhrase_(aiList[0])
  log('AI SEO Enhancer: cleaned English extraction count=' + aiList.length)
  log('AI SEO Enhancer: AI-cleaned primary="' + (aiPrimary || '') + '" list=' + aiList.length)

  if (!aiPrimary && !aiList.length) {
    log('AI SEO Enhancer: keyword cleaning produced empty; using fallback')
 ...
 */
async function runAiSeoEnhancementBatch() {
  loadConfigSecrets_();
  log('AI SEO Enhancer: starting batch...');

  try {
    const trips = await fetchTripsNeedingSeo_(AI_SEO_BATCH_SIZE)

    if (!trips || !trips.length) {
      log('AI SEO Enhancer: no trips with AI_SEO_Status = Pending');
      return;
    }
    ...
    for (let i = 0; i < trips.length; i++) {
      ...
      const improvementId = fields._improvementRecordId
      const improvementFields = fields._improvementFields || {}

      try {
        log('AI SEO Enhancer: processing Trip ' + tripId);

        if (!(await helpers.claimStage(tripId, 'SEO', 20 * 60))) {
          let retried = false
          try {
            let impStatus = ''
            if (improvementId) {
              try {
                const impRes = await airtableGet_(AI_IMPROVEMENT_TABLE, { filterByFormula: `RECORD_ID() = '${String(improvementId)}'`, maxRecords: 1 })
                const recs = impRes && impRes.records ? impRes.records : []
                if (recs.length) impStatus = String((recs[0].fields || {}).AI_SEO_Status || '')
              } catch {
              }
            }
            ...
            if (impStatus === 'Pending') {
              const cleared = await helpers.clearStageLeaseIfRecoverableForRequestedStage(tripId, 'SEO')
              if (cleared) {
                log('AI SEO Enhancer: cleared stale stage lease before SEO claim retry for Trip ' + tripId)
                retried = true
              }
            }
          } catch {
          ...
          if (!(retried && (await helpers.claimStage(tripId, 'SEO', 20 * 60)))) {
            log('AI SEO Enhancer: stage already claimed; skipping Trip ' + tripId);
            continue;
          }
          log('AI SEO Enhancer: recovered stale SEO claim; proceeding Trip ' + tripId)
        }

        // 1) Update status to Processing (in Improvement table)
        if (improvementId) {
          await airtableUpdate_(AI_IMPROVEMENT_TABLE, improvementId, { AI_SEO_Status: 'Processing' })
        }

        // 2) Fetch actual Trip record to get full fields
        const tripRecord = await airtableGet_('Trips', {
        ...
        const tripFields = tripRecord.records[0].fields

        // 3) Fetch linked context text
        const U = await buildUnifiedTripContext_(tripId, tripFields)
        ...
        // 4.5) Keyword cleaning (AI-only selection for enhanced fields)
        const cleanedKw = await cleanEnglishKeywordsWithAiSeoEn_(combinedFields, linkedTextBlocks)
        combinedFields._EN_Cleaned_Seo_Focus = cleanedKw.primary || ''
        combinedFields._EN_Cleaned_Seo_Keywords_List = cleanedKw.list || []

        const prompt = buildSeoPromptFromImprovedContent_(combinedFields, linkedTextBlocks)
        ...
        if (aiResult.AI_SEO_FocusKeywords_List !== undefined) {
          ...
          log('AI SEO Enhancer: final AI keyword list raw=' + rawAiCount + ' english=' + englishList.length);
        ...
        if (!aiFocus) {
          if (aiResult.AI_SEO_FocusKeywords) log('AI SEO Enhancer: AI focus keyword rejected as non-English: ' + String(aiResult.AI_SEO_FocusKeywords));
          if (aiResult.AI_SEO_FocusKeywords_List && Array.isArray(aiResult.AI_SEO_FocusKeywords_List) && aiResult.AI_SEO_FocusKeywords_List.length) {
            ...
            if (aiFocus) log('AI SEO Enhancer: recovered English primary keyword from AI keyword list');
          }
          if (!aiFocus && sourceEnglishFocusForRecovery) {
            aiFocus = sourceEnglishFocusForRecovery;
            log('AI SEO Enhancer: recovered English primary keyword from source focus keyword');
          }
          if (!aiFocus && sourceEnglishListForRecovery.length) {
            aiFocus = normalizeEnglishSeoKeywordPhrase_(sourceEnglishListForRecovery[0]);
            if (aiFocus) log('AI SEO Enhancer: recovered English primary keyword from source keyword list');
          }
        }
        ...
        if (!aiResult.AI_SEO_FocusKeywords_List.length && sourceEnglishListForRecovery.length) {
          ...
          log('AI SEO Enhancer: recovered final English keyword list from clean source');
        }
        ...
        log('AI SEO Enhancer: AI extracted phrase count=' + finalGate.rawCount)
        if (finalGate.rejected) log('AI SEO Enhancer: final English gate rejected extracted phrases=' + finalGate.rejected)
        if (finalGate.refillUsed) log('AI SEO Enhancer: refill used clean English-safe pool count=' + finalGate.refillUsed)
        ...
        log('AI SEO Enhancer: final saved keyword list count=' + (Array.isArray(aiResult.AI_SEO_FocusKeywords_List) ? aiResult.AI_SEO_FocusKeywords_List.length : 0))
        ...
        if (aiResult.AI_SEO_Meta_Description) {
          ...
          if (metaRes.trimmedToSentence) log('AI SEO Enhancer: SEO description trimmed to last complete sentence');
          if (metaRes.fallbackUsed) log('AI SEO Enhancer: SEO description fallback complete sentence used');
          if (beforeMeta !== aiResult.AI_SEO_Meta_Description) log('AI SEO Enhancer: SEO meta description cleaned');
        }
        if (!aiResult.AI_SEO_Meta_Description) {
          var fb = ''
          if (originalMetaDesc) {
          ...
          if (fb) log('AI SEO Enhancer: meta description fallback applied')
        }
        ...
        if (improvementId) {
          await airtableUpdate_(AI_IMPROVEMENT_TABLE, improvementId, updateFields)
          // 8) Ensure status is Done
          await airtableUpdate_(AI_IMPROVEMENT_TABLE, improvementId, { AI_SEO_Status: 'Done' })
        }
        ...
        if (tripId) {
          ...
          await airtableUpdate_('Trips', tripId, tripUpdates)
        }

        log('AI SEO Enhancer: completed Trip ' + tripId);

      } catch (recErr) {
        log('AI SEO Enhancer: error processing Trip ' + tripId + ' — ' + recErr.message);
        
        // Log error to improvement record
        if (improvementId) {
          await airtableUpdate_(AI_IMPROVEMENT_TABLE, improvementId, {
            AI_SEO_Status: 'Error',
            AI_SEO_LastUpdated: new Date().toISOString(),
    ...
    log('AI SEO Enhancer: fatal batch error — ' + e.message);
  }

  log('AI SEO Enhancer: batch finished.');
}
 ...
 */
async function fetchTripsNeedingSeo_(limit) {
  var formula = "{AI_SEO_Status} = 'Pending'";

  const res = await airtableGet_(AI_IMPROVEMENT_TABLE, {
...
function buildSeoPromptFromImprovedContent_(fields, linkedTextBlocks) {
  ...
  log('AI SEO Enhancer: source keyword list raw=' + rawCount + ' english=' + englishProvidedList.length);
  ...
  if (originalSeoFocusRaw && !originalSeoFocus) {
    log('AI SEO Enhancer: original focus keyword rejected as non-English: ' + String(originalSeoFocusRaw));
  }
...
function createAiSeoEnhancerTrigger() {
  log('AI SEO: createAiSeoEnhancerTrigger is not supported in Node runtime')
}
 ...
 */
async function fetchLinkedContextTextForTrip_(tripId) {
  if (!tripId) return [];
  ...
  for (var i = 0; i < tables.length; i++) {
    ...
    var res1 = await airtableGet_(tname, {
 ...
 */
async function ensureUniqueKeyword_(aiResult, tripId, improvementId) {
  ...
  if (!(await checkKeywordExists_(originalKeyword, tripId, improvementId))) {
    return;
  }

  log('AI SEO Enhancer: Keyword "' + originalKeyword + '" is duplicate. Resolving...');
  ...
  for (var i = 0; i < relatedKeywords.length; i++) {
    ...
    if (candidate && candidate.toLowerCase() !== originalKeyword.toLowerCase()) {
       if (!(await checkKeywordExists_(candidate, tripId, improvementId))) {
         newKeyword = candidate;
         log('AI SEO Enhancer: Resolved with related keyword: ' + newKeyword);
         break;
       }
    }
  }
  ...
  if (!newKeyword) {
    ...
    if (!checkKeywordExists_(candidateYear, tripId, improvementId)) {
      ...
      log('AI SEO Enhancer: Resolved with year: ' + newKeyword);
    }
  }
  ...
  if (!newKeyword) {
    ...
    for (var j = 0; j < suffixes.length; j++) {
      ...
      if (!checkKeywordExists_(candidateSuffix, tripId, improvementId)) {
        ...
        log('AI SEO Enhancer: Resolved with suffix: ' + newKeyword);
        break;
      }
    }
  }
  ...
  if (!newKeyword) {
     ...
     log('AI SEO Enhancer: Resolved with fallback: ' + newKeyword);
  }
 ...
 */
async function checkKeywordExists_(keyword, currentTripId, currentImprovementId) {
  ...
  var res1 = await airtableGet_('Trips', {
  ...
  if (currentImprovementId) {
    ...
    var res2 = await airtableGet_(AI_IMPROVEMENT_TABLE, {
...
function createSeoEnhancer(options) {
  let inited = false
  async function ensureInit() {
    if (inited) return
    initSeoEnhancer(options)
    inited = true
  }

  return {
    runAiSeoEnhancementBatch: async (...args) => {
      await ensureInit()
      return runAiSeoEnhancementBatch(...args)
    }
  }
}

module.exports = { createSeoEnhancer }
