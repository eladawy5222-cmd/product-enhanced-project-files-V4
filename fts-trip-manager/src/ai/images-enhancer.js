...
function initImagesEnhancer(options) {
  if (!options) throw new Error('createImagesEnhancer: missing options')
  ...
  if (!airtable) throw new Error('createImagesEnhancer: missing options.airtable')
  if (!http) throw new Error('createImagesEnhancer: missing options.http')
  if (!config) throw new Error('createImagesEnhancer: missing options.config')
  if (!logger) throw new Error('createImagesEnhancer: missing options.logger')
  if (!store) throw new Error('createImagesEnhancer: missing options.store')
  if (!aiProvider) throw new Error('createImagesEnhancer: missing options.aiProvider')
  ...
  enhancement = createEnhancementHelpers({ airtable, lock, store, logger, config })
}
...
async function callAi_(prompt) {
  ...
  throw new Error('AI Images: aiProvider missing callOpenai/callOpenAiJson')
}
...
async function buildUnifiedTripContext_(tripId, tripFields) {
  ...
  const highlights = await fetchRecordsByTrip_('Highlights Improvement With AI', tripId, tripPublicId, 50, tripName)
  const itinerary = await fetchRecordsByTrip_('Itinerary Improvement With AI', tripId, tripPublicId, 100, tripName)
  const includes = await fetchRecordsByTrip_('TripIncludes Improvement With AI', tripId, tripPublicId, 100, tripName)

  const highlightsText = highlights
    .map((r) => String((r.fields || {}).AI_Highlight || '').trim())
    .filter(Boolean)
    .join('\n')
  ...
  const includesText = includes
    .map((r) => {
      const f = r.fields || {}
      return String(f.AI_IncludeItem || f.AI_IncludeText || '').trim()
    })
    .filter(Boolean)
    .join('\n')

  return { highlightsText, itineraryText, includesText }
}
...
function makeUniqueTitleWithCounter_AiImages_(baseTitle, usedKeys, limit) {
  ...
  for (var i = 0; i < variants.length; i++) {
    var w = variants[i];
    var c1 = truncateByWords_AiImages_(String(base + ' ' + w).trim(), maxLen);
    var k1 = normalizeTitleKey_AiImages_(c1);
    if (k1 && !usedKeys[k1]) return c1;

    var c2 = truncateByWords_AiImages_(String(w + ' ' + base).trim(), maxLen);
    var k2 = normalizeTitleKey_AiImages_(c2);
    if (k2 && !usedKeys[k2]) return c2;
  }

  var words = String(base).split(/\s+/).filter(function(x) { return !!x; });
  if (words.length > 3) {
    var shortened = truncateByWords_AiImages_(words.slice(0, 3).join(' '), maxLen);
    var ks = normalizeTitleKey_AiImages_(shortened);
    if (ks && !usedKeys[ks]) return shortened;
  }
...
async function callOpenAiVisionForImageMeta_AiImages_(imageUrl, ctx, keywordPlan, opts) {
  ...
  if (!aiProvider) throw new Error('AI Images: missing aiProvider')
  var aiResult = null
  if (typeof aiProvider.callOpenaiVisionJson === 'function') {
    aiResult = await aiProvider.callOpenaiVisionJson(prompt, url)
  } else {
    aiResult = await callAi_(prompt)
  }
  if (!aiResult) throw new Error('AI metadata: empty result')
  if (typeof aiResult === 'string') {
    try {
      return JSON.parse(aiResult)
    } catch {
      var clean = String(aiResult || '').replace(/```json/g, '').replace(/```/g, '').trim();
 ...
 * MAIN BATCH FUNCTION
 ************************************************************/
async function runAiImagesEnhancementBatch() {
  loadConfigSecrets_();
  log('AI Images: starting batch...');
  ...
  if (!trips || !trips.length) {
    log('AI Images: no trips with ' + IMAGES_STATUS_FIELD + " = 'Pending'.");
    return;
  }
  ...
  for (let t = 0; t < trips.length; t++) {
    ...
    try {
      ...
      await updateTripImagesStatus_(tripId, 'Processing')
      
      // 2) Get TripID from trip fields
      var tripNumber = tripFields.TripID || '';
      if (!tripNumber) {
        log('AI Images: Trip ' + tripId + ' has no TripID field');
        await updateTripImagesStatus_(tripId, 'Error')
        continue
      }
      
      log('AI Images: searching for images with SourceTrip = ' + tripNumber);
      
      // 3) Delete old improved records for this trip
      await deleteOldImprovedImagesForTrip_(tripId, tripNumber)
      ...
      log('AI Images: searching with formula: ' + imagesFormula);
      
      var imagesParams = {
        filterByFormula: imagesFormula
      };
      
      var imagesRes = await airtableGet_(IMAGES_TABLE, imagesParams)
      ...
      if (allAtts.length) {
        log('AI Images: syncing ' + allAtts.length + ' attachments from Trip fields...');

        for (let a = 0; a < allAtts.length; a++) {
          const entry = allAtts[a]
          ...
          if (filename && existingTitles.indexOf(filename) !== -1) continue;
          if (existingUrls.indexOf(url) !== -1) continue;
          ...
          try {
            ...
            var createRes = await airtableCreate_(IMAGES_TABLE, newImageFields)
            ...
            if (newRec && newRec.id) {
              ...
              log('AI Images: created Image record ' + newRec.id + ' from ' + entry.role);
            }
          } catch (e) {
            log('AI Images: failed to create Image record from ' + entry.role + ' — ' + e.message);
          }
        }
      }
      
      if (!images || !images.length) {
        log('AI Images: no images found for Trip ' + tripId);
        await updateTripImagesStatus_(tripId, 'Done')
        continue
      }
      
      log('AI Images: found ' + images.length + ' images for Trip ' + tripId);
      
      var processedCount = 0;
      var errorCount = 0;

      var tripImprovement = await getTripImprovementCached_AiImages_(tripId, tripFields)
      ...
      for (let imgIdx = 0; imgIdx < images.length; imgIdx++) {
        const imageRec = images[imgIdx]
        var imageId = imageRec.id;
        var imageFields = imageRec.fields || {};

        try {
          var ctx = await buildImageContext_(imageFields, tripFields, imageId, tripId)
          ...
          if (titleKey && usedTitleKeys[titleKey]) {
            try {
              if (imageUrl) {
                var retry = await callOpenAiVisionForImageMeta_AiImages_(imageUrl, ctx, (typeof kwPlanForImage !== 'undefined' ? kwPlanForImage : { all: (tripKeywordPlans && tripKeywordPlans.all) ? tripKeywordPlans.all : [], primary: basePrimary, secondary: [] }), { forbiddenTitles: usedTitles, preferredTitleKeyword: preferredTitleKeywordForImage })
                await sleep(2000)
                ...
                var retryRes = await callAi_(retryPrompt)
                await sleep(2000)
                if (typeof retryRes === 'string') {
                  try { retryRes = JSON.parse(retryRes); } catch (eJson) { retryRes = null; }
                }
          ...
          await airtableCreate_(IMAGES_IMPROVEMENT_TABLE, fieldsCreate)
          
          processedCount++;
          
        } catch (e) {
          log('AI Images: error for Image ' + imageId + ' — ' + e.message);
          errorCount++;
        }
      }
      
      log('AI Images: Trip ' + tripId + ' → processed ' + processedCount + ' images (' + errorCount + ' errors)');
      
      // 4) Update trip status
      await updateTripImagesStatus_(tripId, 'Done')
      
    } catch (e) {
      log('AI Images: error for Trip ' + tripId + ' — ' + e.message);
      await updateTripImagesStatus_(tripId, 'Error')
    }
  }
  
  log('AI Images: batch finished.');
}
...
async function buildImageContext_(imageFields, tripFields, imageId, tripId) {
  ...
  var imp = await getTripImprovementCached_AiImages_(tripId, tripFields)
  if (imp) {
...
async function getTripImprovementCached_AiImages_(tripId, tripFields) {
  ...
  try {
    ...
    if ((!impRes || !impRes.records || !impRes.records.length) && tripFields && tripFields['Improvement With AI']) {
      ...
      if (Array.isArray(impLinked) && impLinked.length > 0) {
        ...
        impRes = await airtableGet_(IMPROVEMENT_TABLE, { filterByFormula: "RECORD_ID() = '" + impRecId + "'" })
      }
    }
 ...
 */
async function deleteOldImprovedImagesForTrip_(tripId, tripNumber) {
  if (!tripId) return;
...
async function updateTripImagesStatus_(tripId, status) {
  if (!tripId) return;
  var fields = {};
 ...
 * TRIGGER SETUP
 ************************************************************/
async function createAiImagesEnhancerTrigger() {
  log('AI Images: createAiImagesEnhancerTrigger is not supported in Node runtime')
}

function createImagesEnhancer(options) {
  let inited = false
  async function ensureInit() {
    if (inited) return
    initImagesEnhancer(options)
    inited = true
  }

  return {
    runAiImagesEnhancementBatch: async (...args) => {
      await ensureInit()
      return runAiImagesEnhancementBatch(...args)
    }
  }
}

module.exports = { createImagesEnhancer }
