...
function throttleStage_Updater_(label, ms) {
  if (!UPDATER_ENABLE_STAGE_THROTTLING) return;
  var waitMs = Math.max(0, Number(ms) || 0);
  if (!waitMs) return;
  Logger.log('Updater: Throttling before ' + label + ' for ' + waitMs + ' ms');
  sleep_Updater_(waitMs);
}
...
function runNonCriticalStage_Updater_(label, fn, delayMs) {
  ...
  try {
    ...
    if (isQuotaLikeError_Updater_(e)) {
      Logger.log('Updater: Non-critical stage throttled/soft-failed [' + label + ']: ' + (e && e.message ? e.message : String(e)));
      return null;
    }
    throw e;
  }
...
function runUpdaterBatch() {
  ...
  trips.records.forEach(function(tripRec) {
    ...
    try {
      ...
      try {
        throttleStage_Updater_('store SEO validation', UPDATER_SCHEMA_STAGE_DELAY_MS);
      ...
      try {
        throttleStage_Updater_('store translation url map', UPDATER_SCHEMA_STAGE_DELAY_MS);
        var infoForUrl = null;
        try { infoForUrl = getTripInfoFromWpCached_Updater_(primaryWpId); } catch (eUrlPrimary) {}
      ...
      if (languages.length > 1) {
        ...
        for (var i = 0; i < languagesToProcess.length; i++) {
          ...
          try {
            ...
            try {
              if (existingTransId) {
                existingTransWpInfo = getTripInfoFromWpCached_Updater_(existingTransId);
              }
            } catch (eSlugCheck) {
            ...
            if (existingTransId && existingTransWpInfo) {
              ...
              updateOnly = true;
              slugLocked = true;
              Logger.log('TRANSLATION EXISTS - UPDATE ONLY - SLUG LOCKED (' + targetLang + ')');
              Logger.log('EXISTING PERMALINK PRESERVED (' + targetLang + ')');
            } else if (existingTransId && !existingTransWpInfo) {
              Logger.log('TRANSLATION NOT FOUND - CREATE WITH GENERATED SLUG (' + targetLang + ')');
            }
            ...
            if (!purityCheck.ok) {
              Logger.log('LANGUAGE CONTAMINATION DETECTED (' + targetLang + '): ' + JSON.stringify(purityCheck.contaminated));
              Logger.log('REGENERATING CONTAMINATED FIELD (' + targetLang + '): seo_fields');
              translatedPayload = regenerateContaminatedSeoFields_Updater_(translatedPayload, targetLang, providedForLang, specConstraints);
              translatedPayload = enforceCanonicalMuseumFamilyFidelityInPayload_Updater_(translatedPayload, targetLang, specConstraints);
              ...
              if (purityCheck.ok) {
                Logger.log('TARGET LANGUAGE PURITY PASSED (' + targetLang + '): seo_fields');
              }
            } else {
              Logger.log('TARGET LANGUAGE PURITY PASSED (' + targetLang + '): seo_fields');
            }
            ...
            if (translatedPayload && translatedPayload.core && translatedPayload.core.content) {
              ...
              if (bodyCheck.contaminated) {
                ...
                Logger.log('TARGET LANGUAGE PURITY PASSED (' + targetLang + '): body_cleaned');
              } else {
                Logger.log('TARGET LANGUAGE PURITY PASSED (' + targetLang + '): body');
              }
            }
            ...
            if (!updateOnly) {
              ...
              if (slugCheck.ok && slugCheck.accepted_as_good_enough && slugCheck.warnings && slugCheck.warnings.length) {
                Logger.log('SEMANTIC CHECK DOWNGRADED TO SOFT WARNING (' + targetLang + '): ' + slugCheck.warnings.join(', '));
                Logger.log('LOCALIZED SLUG ACCEPTED AS GOOD ENOUGH (' + targetLang + '): ' + candidateSlug);
              }
              if (!slugCheck.ok) {
                Logger.log('NEW SLUG REJECTED AS SEMANTICALLY WEAK (' + targetLang + '): ' + candidateSlug + ' | ' + slugCheck.reasons.join(', '));
                Logger.log('REGENERATING NEW TRANSLATION SLUG (' + targetLang + ')');
                try {
                  var regen = regenerateTitleSlugOnlyForLocalization_Updater_(translatedPayload, translatedData, targetLang, providedForLang, specConstraints, false, { ok: false, found: ['semantic_slug_weak'], severity: 'hard' });
                  if (regen && regen.slug) {
                    ...
                    if (check2.ok) {
                      ...
                      Logger.log('NEW SLUG REJECTED AS SEMANTICALLY WEAK (' + targetLang + '): ' + cand2 + ' | ' + check2.reasons.join(', '));
                    }
                  }
                } catch (eRegenSlug) {}
              }
              if (!slugCheck.ok) {
                ...
                Logger.log('DETERMINISTIC FALLBACK SLUG BUILT (' + targetLang + '): ' + fbSlug);
                if (fbSlug.indexOf('day-tours') === -1 && fbSlug.indexOf('day') === -1 && fbSlug.indexOf('tours') === -1) {
                  Logger.log('DETERMINISTIC FALLBACK SIMPLIFIED (' + targetLang + ')');
                }
                if (fbSlug && fbSlug.indexOf('nmec') !== -1) {
                  Logger.log('MULTILINGUAL FALLBACK REJECTED (' + targetLang + '): contains nmec');
                  fbSlug = '';
                }
                ...
                if (fbCheck.ok) {
                  ...
                  if (fbCheck.reasons && fbCheck.reasons.join(',').indexOf('multilingual_hybrid') !== -1) {
                    Logger.log('MULTILINGUAL FALLBACK REJECTED (' + targetLang + '): ' + fbSlug);
                  }
                  Logger.log('NEW SLUG REJECTED AS SEMANTICALLY WEAK (' + targetLang + '): ' + fbSlug + ' | ' + fbCheck.reasons.join(', '));
                }
              }

              if (!slugCheck.ok) {
                var reasonSlug = 'new_slug_semantically_weak: ' + slugCheck.reasons.join(', ');
                Logger.log('Updater: SKIPPING LANGUAGE ' + targetLang + ' (new slug): ' + reasonSlug);
                translationSkipped.push({ lang: targetLang, message: reasonSlug });
                continue;
              }

              Logger.log('FINAL NEW TRANSLATION SLUG ACCEPTED (' + targetLang + '): ' + candidateSlug);
              ...
              if (candidateSlug) Logger.log('NEW TRANSLATION PERMALINK CREATED WITH FINAL GENERATED SLUG (' + targetLang + '): ' + candidateSlug);
            }
...
function storeSeoValidationOutputs_Updater_(tripId, vRes) {
  var r = vRes || {};
  var fields = {
...
function classifyTripActivities_Updater_(tripId, tripFields, aiImprovementFields) {
  ...
  (function() {
    ...
    if (before.length !== selectedNames.length) Logger.log('Updater: Removed Pyramids & History (no explicit pyramids signal)');
  })();
...
function mapAirtableToWordPress_Updater_(data, tripFields, overrideLang) {
  ...
  try {
    ...
    if (payload.meta && Object.prototype.hasOwnProperty.call(payload.meta, 'rank_math_title') && payload.meta.rank_math_title) {
      ...
      safeRmTitle = upd_strengthenCairoCivilizationSeoTitle_Updater_(safeRmTitle, civCtx, seoFlags, evTitleText);
  ...
  if (data.includes.length > 0) {
    ...
    if (!addonNorms.length) {
      try {
        var cAdd = [];
        var tripNumber0 = tripFields && tripFields.TripID ? String(tripFields.TripID) : '';
        var tripTitle0 = tripFields && tripFields.Title ? String(tripFields.Title) : '';
        if (tripId) cAdd.push("FIND('" + String(tripId).replace(/'/g, "\\'") + "', ARRAYJOIN({Trip}))");
        if (tripNumber0) cAdd.push("FIND('" + tripNumber0.replace(/'/g, "\\'") + "', ARRAYJOIN({Trip}))");
        if (tripTitle0) cAdd.push("FIND('" + tripTitle0.replace(/'/g, "\\'") + "', ARRAYJOIN({Trip}))");
        if (cAdd.length) {
          var fAdd = cAdd.length > 1 ? ("OR(" + cAdd.join(', ') + ")") : cAdd[0];
          var resAdd = airtableGet_('AddOns', { filterByFormula: fAdd, pageSize: 50 });
          addonNorms = (resAdd && resAdd.records ? resAdd.records : []).map(function(r) {
            var f = r && r.fields ? r.fields : {};
            return String(f.AddOnTitle || f.AddOn_Name || f.Name || f.Title || '').toLowerCase().replace(/[^a-z0-9]+/g, ' ').replace(/\s+/g, ' ').trim();
          }).filter(function(x) { return !!x; });
        }
      } catch (eAdd) {}
    }
  ...
  if (wte && wte.tab_content && wte.tab_content['1_wpeditor']) {
    wte.tab_content['1_wpeditor'] = upd_applyGuideTruthToText_Updater_(wte.tab_content['1_wpeditor'], wte.cost.cost_includes || '', wte.cost.cost_excludes || '');
    payload.core.content = wte.tab_content['1_wpeditor'];
    payload.content = wte.tab_content['1_wpeditor'];
  }
  if (wte && wte.tab_content && wte.tab_content['8_wpeditor']) {
    wte.tab_content['8_wpeditor'] = upd_applyGuideTruthToText_Updater_(wte.tab_content['8_wpeditor'], wte.cost.cost_includes || '', wte.cost.cost_excludes || '');
  }
  if (wte && Array.isArray(wte.trip_highlights) && wte.trip_highlights.length) {
    wte.trip_highlights = wte.trip_highlights.map(function (h) {
      ...
      t0 = upd_applyGuideTruthToText_Updater_(t0, wte.cost.cost_includes || '', wte.cost.cost_excludes || '');
  ...
  if (data.faqs.length > 0) {
    ...
    data.faqs.forEach(function(rec) {
      ...
      a0 = upd_removeUnsupportedHighRiskParts_Updater_(a0, seoFlags);
      a0 = upd_fixBrokenFaqText_Updater_(a0);
  ...
  try {
    ...
    if (!payload.meta.faq_schema_data && data.faqs && data.faqs.length) {
      ...
      if (mainEntity.length) {
        ...
        if (isEn) Logger.log('SCHEMA SOURCE RESOLVED (en, FAQPage meta): title=' + (payload.meta.rank_math_title ? 'rank_math_title' : (g.AI_SEO_Title ? 'AI_SEO_Title' : 'missing')) + ' desc=' + (payload.meta.rank_math_description ? 'rank_math_description' : (g.AI_SEO_Meta_Description ? 'AI_SEO_Meta_Description' : 'missing')));
      }
    }
  } catch (eSchema) {}
...
function upd_applyGuideTruthToText_Updater_(text, includesText, excludesText) {
  var s = String(text || '');
  if (!s.trim()) return '';
...
function upd_strengthenCairoCivilizationSeoTitle_Updater_(title, civCtx, flags, evidenceText) {
  var t = String(title || '').replace(/\s+/g, ' ').trim();
  if (!t) return t;
  if (!civCtx) return t;
  var lc = t.toLowerCase();
  if (/\b(nmec|egyptian civilization museum|national museum of egyptian civilization|civilization museum)\b/.test(lc) &&
      /\bcitadel\b/.test(lc) &&
      (/\bold cairo\b/.test(lc) || /\bcoptic\b/.test(lc))) return t;
  if (lc.indexOf('cairo') === -1) return t;
  if (!/\bmuseum\b/.test(lc) || !/\bcitadel\b/.test(lc)) return t;
  var ev = String(evidenceText || '').toLowerCase();
  var hasOldCairo = /\b(old cairo|coptic|hanging church)\b/.test(ev);
  var hasLunch = !!(flags && flags.has_lunch);
  var out = hasOldCairo ? 'Cairo Day Tour: NMEC, Citadel & Old Cairo' : 'Cairo Day Tour: NMEC & Citadel';
...
function upd_applySeoSnippetPolicy_Updater_(baseTitle, baseMeta, tripFields, seoFlags) {
  ...
  if (/\bold$/i.test(seoTitle)) {
    var src = String((tripFields && tripFields.Title) || '') + ' ' + String(baseTitle || '');
    if (/old cairo/i.test(src)) {
      if (seoTitle.length + 6 <= 60) seoTitle = (seoTitle + ' Cairo').replace(/\s+/g, ' ').trim();
      else seoTitle = seoTitle.replace(/\bold$/i, '').replace(/[,&]\s*$/g, '').trim();
    }
  }
...
function upd_detectEntranceTruthFromText_Updater_(includesText, excludesText) {
  var inc = String(includesText || '').toLowerCase();
  var exc = String(excludesText || '').toLowerCase();
  var incHas = /\b(entrance|admission|ticket|tickets|entrance fee|entrance fees)\b/.test(inc) && !/\b(not included|excluded)\b/.test(inc);
  var excHas = /\b(entrance|admission|ticket|tickets|entrance fee|entrance fees)\b/.test(exc);
...
function upd_detectGuideTruthFromText_Updater_(includesText, excludesText) {
  var inc = String(includesText || '').toLowerCase();
  var exc = String(excludesText || '').toLowerCase();
  var hasGuideInc = /\b(egyptologist|tour guide|guide)\b/.test(inc);
  var hasGuideExc = /\b(egyptologist|tour guide|guide)\b/.test(exc);
  var incConditional = hasGuideInc && /\b(if selected|optional|depending on the option)\b/.test(inc);
  var included = hasGuideInc && !incConditional && !hasGuideExc;
  var optional = incConditional;
  return { included: !!included, optional: !!optional && !included, ambiguous: !included && !optional };
}

function upd_applyEntranceTruthToText_Updater_(text, truth) {
...
function upd_stripAddOnMentions_Updater_(text) {
  var s = String(text || '');
  if (!s.trim()) return '';
...
function upd_filterOptionalItemsFromIncludesText_Updater_(includesText, excludesText) {
  ...
  var excKeys = {};
  excLines.forEach(function(x) { excKeys[normalizeForEnglishPhraseScan_Updater_(x)] = true; });
  var out = [];
  for (var i = 0; i < incLines.length; i++) {
    ...
    var k = normalizeForEnglishPhraseScan_Updater_(t);
    if (k && excKeys[k]) continue;
    out.push(t);
  }
  return out.join('\n');
}

function upd_sanitizeFaqArraysAgainstTruth_Updater_(wte, entranceTruth, guideTruth, includesText, excludesText) {
  ...
  function guideLine_() {
    if (guideTruth && guideTruth.included) return "An Egyptologist guide is included as listed in What's Included.";
    if (guideTruth && guideTruth.optional) return "Guiding depends on the option selected.";
    return "";
  }
  ...
  for (var i = 0; i < Math.min(titles.length, answers.length); i++) {
    ...
    a = upd_fixBrokenFaqText_Updater_(a);
    a = upd_finalizeEntranceFeesFaqAnswer_Updater_(q, a, includesText, excludesText);
    a = upd_applyGuideTruthToText_Updater_(a, includesText, excludesText);
    if (isIncExcQ_(qLc)) {
...
function upd_sanitizeHighlightsAgainstTruth_Updater_(wte, entranceTruth, includesText, excludesText) {
  if (!wte || !Array.isArray(wte.trip_highlights)) return;
  wte.trip_highlights = wte.trip_highlights.map(function(h) {
    var t0 = h && h.highlight_text ? String(h.highlight_text) : '';
    t0 = upd_applyEntranceTruthToText_Updater_(t0, entranceTruth);
    t0 = upd_applyGuideTruthToText_Updater_(t0, includesText, excludesText);
    t0 = upd_stripAddOnMentions_Updater_(t0);
    t0 = String(t0 || '').replace(/\s+/g, ' ').trim();
    return { highlight_text: t0 };
  }).filter(function(h) { return h && h.highlight_text; });
}

function upd_sanitizeSeoAgainstTruth_Updater_(payload, entranceTruth, includesText, excludesText) {
  if (!payload || !payload.meta) return;
  if (payload.meta.rank_math_title) {
    var t = upd_applyEntranceTruthToText_Updater_(payload.meta.rank_math_title, entranceTruth);
    t = upd_applyGuideTruthToText_Updater_(t, includesText, excludesText);
    t = upd_stripAddOnMentions_Updater_(t);
    payload.meta.rank_math_title = t;
  }
  if (payload.meta.rank_math_description) {
    var d = upd_applyEntranceTruthToText_Updater_(payload.meta.rank_math_description, entranceTruth);
    d = upd_applyGuideTruthToText_Updater_(d, includesText, excludesText);
    d = upd_stripAddOnMentions_Updater_(d);
    payload.meta.rank_math_description = d;
  }
}

function upd_sanitizeSchemaAgainstTruth_Updater_(payload, entranceTruth, includesText, excludesText) {
  if (!payload || !payload.meta) return;
  function clean_(s) {
    var x = upd_applyEntranceTruthToText_Updater_(s, entranceTruth);
    x = upd_applyGuideTruthToText_Updater_(x, includesText, excludesText);
    x = upd_stripAddOnMentions_Updater_(x);
    return x;
  }
  if (payload.meta.trip_schema_data && typeof payload.meta.trip_schema_data === 'string') {
    try {
...
function upd_applyPublishConsistencyGuard_Updater_(payload) {
  ...
  upd_sanitizeHighlightsAgainstTruth_Updater_(wte, entranceTruth, includesText, excludesText);
  upd_sanitizeFaqArraysAgainstTruth_Updater_(wte, entranceTruth, guideTruth, includesText, excludesText);
  upd_sanitizeSeoAgainstTruth_Updater_(payload, entranceTruth, includesText, excludesText);
  upd_sanitizeSchemaAgainstTruth_Updater_(payload, entranceTruth, includesText, excludesText);
...
function pushToWordPress_Updater_(wpId, payload) {
  ...
  payload = upd_applyPublishConsistencyGuard_Updater_(payload);
...
function extractKeywordsForTargetLanguage_Updater_(keywordsField, targetLang) {
  ...
  if (lang === 'pl') {
    ...
    Logger.log('TARGET BUCKET BUILT (pl): ' + JSON.stringify(rawBucket.slice(0, 20)) + (rawBucket.length > 20 ? (' (+ ' + (rawBucket.length - 20) + ' more)') : ''));
    var purified = [];
    for (var i = 0; i < rawBucket.length; i++) {
      ...
      if (/\b(piramides|piramidele)\b/.test(t) || /\b(de|din)\b/.test(t)) {
        Logger.log('FALSE POSITIVE REMOVED FROM TARGET BUCKET (pl): ' + kw);
        continue;
      }
      var det = '';
      try { det = detectLanguageSafe_Updater_(kw); } catch (eDet2) { det = ''; }
      if (det) {
    ...
    buckets.pl = purified;
    Logger.log('TARGET BUCKET PURIFIED (pl): ' + JSON.stringify(purified.slice(0, 20)) + (purified.length > 20 ? (' (+ ' + (purified.length - 20) + ' more)') : ''));
  }
  var picked = [];
  ...
  if (!picked.length) {
    Logger.log('NO VALID TARGET-LANGUAGE KEYWORD FOUND - USING NATURAL TRANSLATION (' + lang + ')');
    return null;
  }
  ...
  Logger.log('TARGET-LANGUAGE KEYWORDS EXTRACTED (' + lang + '): ' + JSON.stringify(out.all.slice(0, 8)) + (out.all.length > 8 ? (' (+ ' + (out.all.length - 8) + ' more)') : ''));
  if (out.primary) Logger.log('TARGET-LANGUAGE PRIMARY KEYWORD SELECTED (' + lang + '): ' + out.primary);
  Logger.log('FALLBACK NOT USED - TARGET BUCKET NOT EMPTY (' + lang + ')');
  return out;
}
...
function isLikelyLanguageContamination_Updater_(text, targetLang) {
  ...
  var det = null;
  try { det = detectLanguageSafe_Updater_(t.length > 260 ? t.substring(0, 260) : t); } catch (e) {}
  if (det && det !== lang) {
    if (!(lang.indexOf(String(det).toLowerCase()) === 0 || String(det).toLowerCase().indexOf(lang) === 0)) {
      markers.push('detected_lang_' + det);
    }
  }
...
function regenerateBodyHtmlOnly_Updater_(html, targetLang) {
  var lang = String(targetLang || '').toLowerCase();
  var h = String(html || '');
  if (!h) return h;
  var prompt =
...
function translateKeywordPhraseToTargetLang_Updater_(phrase, targetLang) {
  var lang = String(targetLang || '').toLowerCase();
  ...
  var prompt =
    ...
    "OUTPUT JSON:\n" + "{\"keyword\":\"...\"}";
  var res = callAiForTargetLangWithRetry_Updater_(prompt, lang);
  if (res && typeof res === 'object' && !Array.isArray(res) && res.keyword) {
    return String(res.keyword || '').trim();
  }
  return '';
}

function enforceKeywordsTargetLanguagePurity_Updater_(kwObj, targetLang, englishFallback) {
  var lang = String(targetLang || '').toLowerCase();
  ...
  function isBad_(s) {
    ...
    var det = null;
    try { det = detectLanguageSafe_Updater_(v); } catch (e) {}
    if (det && det !== lang) return true;
    return false;
  }

  if (out.primary && isBad_(out.primary)) {
    Logger.log('LANGUAGE CONTAMINATION DETECTED (' + lang + '): primary_keyword=' + out.primary);
    Logger.log('REGENERATING CONTAMINATED FIELD: primary_keyword (' + lang + ')');
    var src = fb.primary ? fb.primary : out.primary;
    var tr = translateKeywordPhraseToTargetLang_Updater_(src, lang);
  ...
  if (out.primary && out.primary.length > 80) out.primary = out.primary.substring(0, 80).trim();
  out.all = [out.primary].concat(out.secondary).filter(function(x) { return !!String(x || '').trim(); });
  return out;
}
  ...
  function fixText_(txt) {
    ...
    if (lang === 'de') {
      if (/(?:ägyptisch|aegyptisch)\w*\s+museum/i.test(t)) {
        ...
        Logger.log('LANDMARK FIDELITY FIXED IN FIELD (de): aegyptisch* museum -> civilization_museum');
        return t;
      }
    }
    ...
    if (egypt) {
      ...
      if (eNorm && norm.indexOf(eNorm) !== -1) {
        ...
        Logger.log('LANDMARK FIDELITY FIXED IN FIELD (' + lang + '): egyptian_museum -> civilization_museum');
        return t;
      }
    }
    if (norm.indexOf('egyptian museum') !== -1) {
      t = t.replace(/egyptian\s+museum/ig, civ).trim();
      Logger.log('LANDMARK FIDELITY FIXED IN FIELD (' + lang + '): English egyptian museum -> civilization_museum');
      return t;
    }
    if (t.length < 70) {
...
function regenerateContaminatedSeoFields_Updater_(payload, targetLang, kw, spec) {
  var lang = String(targetLang || '').toLowerCase();
  var out = payload || {};
  out.core = out.core || {};
  out.meta = out.meta || {};
...
function getMandatorySeoKeywordsForLang_Updater_(tripFields, enhancedData, targetLang) {
  ...
  merged = enforceKeywordsTargetLanguagePurity_Updater_(merged, lang, englishFallback);
  return merged;
}
...
function removeStandaloneKeywordParagraphs_Updater_(html, primaryKeyword, targetLang) {
  ...
  if (reH2Exact.test(s)) {
    ...
    Logger.log('RAW KEYWORD PARAGRAPH REMOVED (' + String(targetLang || '').toLowerCase() + ')');
  }

  if (s !== before) {
    Logger.log('PRIMARY KEYWORD INTEGRATED NATURALLY (' + String(targetLang || '').toLowerCase() + ')');
  }
  return s;
}
    ...
    if (normalizeForSpecMatch_Updater_(civ) && h2Norm.indexOf(normalizeForSpecMatch_Updater_(civ)) === -1) {
      ...
      if (eNorm && h2Norm.indexOf(eNorm) !== -1) {
        ...
        if (replaced !== h2) {
          ...
          Logger.log('BODY HEADING HARMONIZED WITH MAIN TITLE LANDMARK (' + lang + ')');
          Logger.log('LANDMARK FIDELITY PROPAGATED TO CONTENT INTRO (' + lang + ')');
          return out;
        }
      }
    }
...
function generateTripSchema_Updater_(tripData, targetLang, opts) {
  ...
  if (lang === 'en') {
    ...
    Logger.log('SCHEMA SOURCE RESOLVED (en, TouristTrip): title=' + titleSrc + ' desc=' + descSrc);
    if (!aiSeoDesc && meta.rank_math_description && (core.content_html || core.excerpt)) Logger.log('SCHEMA BYPASSED STALE FALLBACK (en, TouristTrip): used rank_math_description');
    if (!aiSeoTitle && seoTitleRaw && core.title && String(core.title).trim() !== String(seoTitleRaw).trim()) Logger.log('SCHEMA BYPASSED STALE FALLBACK (en, TouristTrip): used rank_math_title');
  }
  ...
  if (!offers) {
    ...
    Logger.log('SCHEMA PRICES SOURCE RESOLVED (en, TouristTrip): source=' + offersResolved.source + ' offers=' + (offers['@type'] || 'Offer') + ' count=' + String(offersResolved.offerCount || 0));
...
function generateFaqSchema_Updater_(enhancedTripData, wpTripInfo, targetLang) {
  ...
  if (lang === 'en') {
    ...
    Logger.log('SCHEMA SOURCE RESOLVED (en, FAQPage): title=' + titleSrc2 + ' desc=' + descSrc2);
    if (meta.rank_math_description) Logger.log('SCHEMA BYPASSED STALE FALLBACK (en, FAQPage): used rank_math_description');
  }
    ...
    if (pkgRecords.length > 0) {
        pkgRecords.forEach(function(pkg) {
           ...
           if (existingWpPkgId) {
             ...
             Logger.log('CACHE HIT - CREATE SKIPPED, UPDATE CONTINUES (package ' + String(targetLang || '') + '): ' + existingWpPkgId);
           } else {
             Logger.log('NEW OBJECT CREATED BECAUSE NO EXISTING TARGET FOUND (package ' + String(targetLang || '') + '): airtable_pkg=' + pkId);
           }
           Logger.log('Updater: Sending package "' + payload.title + '" to WP...');
           Logger.log('Updater: Package copy -> excerpt_chars=' + (payload.excerpt ? String(payload.excerpt).length : 0) + ', content_html_chars=' + (payload.content_html ? String(payload.content_html).length : 0));
           var sendRes = sendPackageToWp_Updater_(payload, { lang: targetLang });
           var newId = sendRes && sendRes.id ? String(sendRes.id) : '';
           if (!newId && existingWpPkgId && sendRes && String(sendRes.error_code || '') === 'not_found') {
             Logger.log('STALE PACKAGE ID DETECTED (package ' + String(targetLang || '') + '): ' + existingWpPkgId + ' -> attempting recreate');
             var createPayload = JSON.parse(JSON.stringify(payload || {}));
             delete createPayload.id;
             var createRes = sendPackageToWp_Updater_(createPayload, { lang: targetLang });
             if (createRes && createRes.id) {
               newId = String(createRes.id);
               Logger.log('PACKAGE RECREATED AFTER NOT_FOUND (package ' + String(targetLang || '') + '): newId=' + newId);
             } else {
               Logger.log('PACKAGE RECREATE FAILED AFTER NOT_FOUND (package ' + String(targetLang || '') + '): ' + JSON.stringify(createRes || {}));
             }
           }
           
           if (newId) {
              if (existingWpPkgId && String(existingWpPkgId) === String(newId)) {
                Logger.log('EXISTING OBJECT REUSED AND REFRESHED (package ' + String(targetLang || '') + '): ' + newId);
              } else {
              ...
              if (!skipAirtableSync) {
                try {
                    if (!existingWpPkgId || String(existingWpPkgId) !== String(newId)) {
                      ...
                      Logger.log('EXISTING TRANSLATED CONTENT REFRESHED (package airtable record ' + pkg.id + ')');
                    }
                } catch (e) {
       ...
       var res2 = sendPackageToWp_Updater_(payload, { lang: targetLang });
...
function sendPackageToWp_Updater_(payload, opts) {
      ...
      var json = null;
      try { json = JSON.parse(text); } catch (eJson) { json = { raw: text }; }
      var errCode = (json && json.code) ? String(json.code) : '';

      if (json && json.id) {
         if (isUpdate) Logger.log('Updater: Updated Package ' + json.id);
         else Logger.log('Updater: Created Package ' + json.id);
         return { id: json.id, http_code: code, error_code: errCode, body: json };
      } else if (isUpdate && code >= 200 && code < 300) {
         Logger.log('Updater: Updated Package ' + String(payload.id));
         return { id: payload.id, http_code: code, error_code: errCode, body: json };
      } else {
         Logger.log('Updater: Failed to send package (code=' + code + '): ' + JSON.stringify(json));
         return { id: null, http_code: code, error_code: errCode, body: json };
      }
}
       ...
       impRecords.forEach(function(imgRec) {
           ...
           if (wpMediaId) {
              ...
              if (!declaredType) {
               ...
               var mediaJson2 = null;
               try { mediaJson2 = getMediaFromWordPress_Updater_(String(wpMediaId)); } catch (eMeta2) { mediaJson2 = null; }
               ...
               if (pref && pref._source === 'airtable_ai') {
                 Logger.log('ENGLISH IMAGE METADATA SOURCE: airtable_ai');
                 if (pref._partial_merge) Logger.log('ENGLISH IMAGE METADATA PARTIAL MERGE APPLIED');
                 Logger.log('ENGLISH IMAGE METADATA FALLBACK SKIPPED BECAUSE AI TABLE HAS DATA');
                 ...
                 Logger.log('ENGLISH IMAGE METADATA SOURCE: fallback_ai');
                 ensureEnglishMediaMetadataForAttachment_Updater_(wpMediaId, tripFields, tripId, declaredType, '');
               } else {
                 Logger.log('ENGLISH IMAGE METADATA SOURCE: wordpress_existing');
               }
           }
       });
   }
...
function storeTranslationUrlMap_Updater_(tripRecId, map) {
  try {
...
function pushTranslationUrlMapMetaToWordPress_Updater_(wpTripId, map) {
  try {
    pushToWordPress_Updater_(wpTripId, { meta: { translation_url_map: JSON.stringify(map || {}) } });
  } catch (e) {}
}
...
function ensureTranslatedAttachmentWithStatus_Updater_(tripRecId, map, sourceId, lang, imageRole) {
  ...
  if (cached) {
    var exists = null;
    try { exists = getMediaFromWordPress_Updater_(cached); } catch (eExists) { exists = null; }
    if (exists) {
      Logger.log('CACHE HIT - CREATE SKIPPED, UPDATE CONTINUES (attachment ' + lang + '): ' + sourceId + ' -> ' + cached);
      return { id: cached, status: 'reuse', role: imageRole || '' };
    }
    Logger.log('CACHE HIT BUT TARGET MISSING - CREATE CONTINUES (attachment ' + lang + '): ' + sourceId + ' -> ' + cached);
  }
  ...
  Logger.log('NEW OBJECT CREATED BECAUSE NO EXISTING TARGET FOUND (attachment ' + lang + '): ' + sourceId + ' -> ' + newId);
...
function translateImageMetadataForLanguage_Updater_(imageMeta, targetLang, seoData, imageRole, opts) {
  ...
  if (role === 'featured') {
    ...
    if (altKeyword) {
      Logger.log('FEATURED ALT KEYWORD SELECTED (' + lang + '): ' + altKeyword);
      if (!contains_(out.alt, altKeyword)) {
        ...
        if (alt2) {
          out.alt = alt2;
          Logger.log('FEATURED ALT KEYWORD INCLUDED (' + lang + '): ' + altKeyword);
        } else {
          Logger.log('FEATURED ALT KEYWORD SKIPPED (NO ROOM) (' + lang + '): ' + altKeyword);
        }
      } else {
        Logger.log('FEATURED ALT KEYWORD ALREADY PRESENT (' + lang + '): ' + altKeyword);
      }
    } else {
      Logger.log('FEATURED ALT KEYWORD SKIPPED (NOT SUITABLE) (' + lang + ')');
    }
  }
  ...
  if (!purity.ok) {
    Logger.log('LANGUAGE CONTAMINATION DETECTED (' + lang + '): image_metadata ' + JSON.stringify(purity.contaminated));
    Logger.log('REGENERATING CONTAMINATED FIELD: image_metadata (' + lang + ')');
    var forbid =
    ...
    var prompt2 =
      ...
      "primary_keyword: " + primary + "\n" +
      "secondary_keywords: " + JSON.stringify(secondary) + "\n";
    var res2 = callAiForTargetLangWithRetry_Updater_(prompt2, lang);
    if (res2 && typeof res2 === 'object' && !Array.isArray(res2)) {
      ...
      Logger.log('IMAGE METADATA LANGUAGE PURITY FIXED (' + lang + ')');
    }
  } else {
    Logger.log('TARGET LANGUAGE PURITY PASSED (' + lang + '): image_metadata');
  }
...
function enforceCanonicalMuseumFamilyFidelityInImageMetadata_Updater_(translatedMeta, targetLang, spec) {
  ...
  function fixField_(val) {
    ...
    if (/(?:\begyptian\s+museum\b)/i.test(t)) {
      ...
      Logger.log('IMAGE ENTITY FIDELITY FIXED (' + lang + '): english_egyptian_museum -> civilization_museum');
      return t;
    }
    if (egypt && new RegExp('\\b' + escapeRegex_Updater_(egypt) + '\\b', 'i').test(t)) {
      t = t.replace(new RegExp(escapeRegex_Updater_(egypt), 'ig'), civ).trim();
      Logger.log('IMAGE ENTITY FIDELITY FIXED (' + lang + '): localized_egyptian_museum -> civilization_museum');
      return t;
    }
    if (/(?:ägyptisch|aegyptisch)\w*\s+museum/i.test(t)) {
      t = t.replace(/(?:ägyptisch|aegyptisch)\w*\s+museum/ig, civ).trim();
      Logger.log('IMAGE ENTITY FIDELITY FIXED (' + lang + '): aegyptisch* museum -> civilization_museum');
      return t;
    }
    var n = norm_(t);
