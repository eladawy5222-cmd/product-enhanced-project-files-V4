...
function generateTripSchema_Updater_(tripData, targetLang, opts) {
  ...
  if (lang === 'en') {
    ...
    if (meta.rank_math_description && (core.content_html || core.excerpt)) Logger.log('SCHEMA BYPASSED STALE FALLBACK (en, TouristTrip): used rank_math_description');
    if (seoTitleRaw && core.title && String(core.title).trim() !== title) Logger.log('SCHEMA BYPASSED STALE FALLBACK (en, TouristTrip): used rank_math_title');
  }
