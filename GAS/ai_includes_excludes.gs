...
function generateSafeFallbackExcludes_(linkedTextBlocks, incSet, excSet, limit, rawExcludesCount) {
  ...
  candidates.push('Optional extras (available at an additional cost)');
    ...
    if (!addOns.length) {
      try {
        ...
        if (resRaw && resRaw.records && resRaw.records.length) {
          ...
          Logger.log('AI Inc/Exc: Raw AddOns fallback added ' + addOns.length + ' item(s).');
        }
      } catch (e2) {}
    }
...
function filterHighRiskIncludesNotInRaw_(includes, rawIncludes, rawExcludes, tripId) {
  ...
  if (removed > 0) Logger.log('✅ High-risk includes filtered [Trip ' + tripId + ']: removed ' + removed + ' item(s) not present in raw includes');
  return out;
}
