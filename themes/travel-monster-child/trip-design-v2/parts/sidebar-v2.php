        ...
        <div class="fts-v2-booking-card">
            ...
            <?php if ( ! empty( $sidebar_trust_items ) && is_array( $sidebar_trust_items ) ) :
                ...
                foreach ( $sidebar_trust_items as $ti ) {
                    ...
                    if ( $txt === '' ) continue;
                    $type = isset( $ti['type'] ) ? (string) $ti['type'] : '';
                    $trust_rendered[] = array( 'type' => $type, 'text' => $txt );
                }
                if ( ! empty( $trust_rendered ) ) :
            ?>
            <div class="fts-v2-booking-trust">
                <?php foreach ( $trust_rendered as $ti ) : ?>
                <div class="fts-v2-booking-trust-item">
                    <?php echo wp_kses_post( $icons[ $ti['type'] ] ?? $icons['check'] ); ?>
