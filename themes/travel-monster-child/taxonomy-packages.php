<?php
/**
 * Template for displaying Package taxonomy archive
 * Uses the same structure as WP Travel Engine archive
 */

get_header();

$current_package = get_queried_object();
?>
...
<div class="wp-travel-engine-archive-outer-wrap collapsible-filter-panel">
    ...
    <div class='advanced-search-wrapper package-filter' id="package-trip-filters">
        ...
        <div class="sidebar">
            ...
            <?php
            // فلتر الوجهات للـ Package الحالي مع hierarchy
            ...
            <div class='advanced-search-field search-trip-type wte-list-opn'>
                ...
                <div class="filter-section-content">
                    <ul class="wte-search-terms-list">
                        <?php 
                        function display_destination_hierarchy_single($destinations, $current_package, $parent_id = 0, $level = 0) {
                            foreach ($destinations as $destination) :
                                if ($destination->parent != $parent_id) continue;
                                
                                // حساب عدد الرحلات في هذه الوجهة داخل الـ Package الحالي
                            ...
                            <?php 
                                ...
                                // عرض الـ children
                                display_destination_hierarchy_single($destinations, $current_package, $destination->term_id, $level + 1);
                            endforeach;
                        }
                        ...
                        display_destination_hierarchy_single($all_destinations, $current_package);
                        ?>
                    </ul>
                </div>
            </div>
            ...
            <?php
            // فلتر الأنشطة للـ Package الحالي
            $package_activities = get_terms(array(
                'taxonomy' => 'activities',
                'hide_empty' => false,
            ));
            ...
            <div class='advanced-search-field search-trip-type wte-list-opn'>
                ...
                <div class="filter-section-content">
                    <ul class="wte-search-terms-list">
                        <?php foreach ($package_activities as $activity) :
                            // حساب عدد الرحلات في هذا النشاط داخل الـ Package الحالي
                        ...
                        <li>
                            <label>
                                <input type="checkbox" value="<?php echo esc_attr($activity->slug); ?>" name="activities" class="activities package-filter-item">
            ...
            <?php
            // فلتر أنواع الرحلات للـ Package الحالي
            $package_trip_types = get_terms(array(
                'taxonomy' => 'trip_types',
                'hide_empty' => false,
            ));
            
            if (!empty($package_trip_types)) :
            ?>
            <div class='advanced-search-field search-trip-type wte-list-opn'>
                <h3 class='filter-section-title trip-type'>Trip Types</h3>
                <div class="filter-section-content">
                    <ul class="wte-search-terms-list">
                        <?php foreach ($package_trip_types as $trip_type) :
                            // حساب عدد الرحلات في هذا النوع داخل الـ Package الحالي
                        ...
                        <li>
                            <label>
                                <input type="checkbox" value="<?php echo esc_attr($trip_type->slug); ?>" name="trip_types" class="trip_types package-filter-item">
            ...
            <?php
            // فلتر صعوبة الرحلات للـ Package الحالي
            $package_difficulties = get_terms(array(
                'taxonomy' => 'difficulty',
                'hide_empty' => false,
            ));
            
            if (!empty($package_difficulties)) :
            ?>
            <div class='advanced-search-field search-trip-type wte-list-opn'>
                <h3 class='filter-section-title trip-type'>Difficulties</h3>
                <div class="filter-section-content">
                    <ul class="wte-search-terms-list">
                        <?php foreach ($package_difficulties as $difficulty) :
                            // حساب عدد الرحلات في هذه الصعوبة داخل الـ Package الحالي
                        ...
                        <li>
                            <label>
                                <input type="checkbox" value="<?php echo esc_attr($difficulty->slug); ?>" name="difficulty" class="difficulty package-filter-item">
            ...
            <?php
            // فلتر المدة (Duration) - Dynamic للـ Package الحالي
            // جلب جميع المدد من الرحلات في الـ Package الحالي
    ...
    <div class="wp-travel-engine-archive-repeater-wrap">
        <?php
        // إظهار الهيدر فلتر في صفحة Package
        ...
        <div class="package-header">
            <h1 class="page-title">
                <?php 
                $package_name = esc_html($current_package->name);
                // إزالة "Egypt Tours Package" من الاسم
                $package_name = str_replace(array('Egypt Tours Package', 'Egypt Tours', 'Package'), '', $package_name);
                $package_name = trim($package_name);
                echo $package_name;
                ?>
            </h1>
            <?php if ($current_package->description) : ?>
                <div class="package-description">
                    <p><?php echo esc_html($current_package->description); ?></p>
                </div>
            <?php endif; ?>
        </div>
        ...
        <div class="wte-category-outer-wrap">
            ...
            // Custom query for package trips
            ...
            if ($package_trips->have_posts()) :
                ...
                echo '<h3>' . sprintf(__('No trips found in %s package.', 'wp-travel-engine'), $current_package->name) . '</h3>';
                echo '<p>' . __('Please check back later or browse our other packages.', 'wp-travel-engine') . '</p>';
                echo '</div>';
            endif;
            ...
            ?>
                <div class="wte-load-more-wrapper">
                    <button type="button" class="wte-load-more-btn" 
                            ...
                            data-package-slug="<?php echo $current_package->slug; ?>">
...
jQuery(document).ready(function($) {
    ...
    // دالة فلترة الرحلات للـ Package الحالي
    function filterPackageTrips() {
        var selectedFilters = {
            destination: [],
            activities: [],
            trip_types: [],
        ...
        $.ajax({
            ...
            type: 'POST',
            data: {
                action: 'filter_single_package_trips',
                filters: selectedFilters,
                package_slug: '<?php echo $current_package->slug; ?>',
                nonce: '<?php echo wp_create_nonce('filter_single_package_trips'); ?>'
            },
            beforeSend: function() {
    ...
    $('.wte-load-more-btn').on('click', function() {
        ...
        var packageSlug = $btn.data('package-slug');
